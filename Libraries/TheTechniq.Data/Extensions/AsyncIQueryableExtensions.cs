...
public static class AsyncIQueryableExtensions
{
    ...
    public static IOrderedQueryable<T> SortBy<T>(this IQueryable<T> source, string secondSort, string firstSort = "SortOrder")
    {
        ...
        var resultExp = Expression.Call(typeof(Queryable), "OrderBy", [typeof(T), typeof(bool)], source.Expression, Expression.Quote(orderByNullLambda));
        var resultExp1 = Expression.Call(typeof(Queryable), "ThenBy", [typeof(T), propertyInfo.PropertyType], resultExp, Expression.Quote(orderByNullLambda1));
        ...
        var resultExp2 = Expression.Call(typeof(Queryable), "ThenBy", [typeof(T), propertyInfo2.PropertyType], resultExp1, Expression.Quote(orderByNullLambda2));
    ...
    public static async Task<IPagedList<T>> ToPagedListAsync<T>(this IQueryable<T> source, int pageIndex, int pageSize, 
        bool getOnlyTotalCount = false, bool ignorePaging = false)
    {
        if (source == null)
    ...
    public static async Task<IQueryable<TSource>> OrderByAsync<TSource>(this IQueryable<TSource> dataSource, string propertyName, bool IsOrderByDescending = false)
    {
        ...
        LambdaExpression orderByExpression = Expression.Lambda(property, param);

        // Construct the call to OrderBy method with the type of the property
        MethodCallExpression orderByCallExpression = Expression.Call(
            typeof(Queryable),
            IsOrderByDescending ? "OrderByDescending" : "OrderBy",
            [typeof(TSource), property.Type],
            dataSource.Expression,
            Expression.Quote(orderByExpression)
        );

        // Create an expression tree and compile it to a delegate
        var sortedDataQuery = dataSource.Provider.CreateQuery<TSource>(orderByCallExpression);

        // Execute the query asynchronously and return the sorted data
        return await Task.FromResult(sortedDataQuery);
    }

    public static IQueryable<TSource> OrderBy<TSource>(this IQueryable<TSource> dataSource, string propertyName, bool IsOrderByDescending = false)
    {
        // Create a parameter expression for the entity type
        ParameterExpression param = Expression.Parameter(typeof(TSource), "x");
        ...
        // Execute the query and return the sorted data
        return sortedDataQuery;
    }
}