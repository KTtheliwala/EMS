...
namespace TheTecniQ.API.Infrastructure.Extensions
{
    public class ValidateModelStateAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            if (queryParameters.Count > 0 && queryParameters.Any(queryParameter => !parameters.Any(p => p.Name.TrimStart().TrimEnd().ToLower() == queryParameter.TrimStart().TrimEnd().ToLower())))
            {
                context.Result = new BadRequestObjectResult(APIResponseExtensions.GenerateResponse(ApiStatusCode.Status400BadRequest, "Bad Request."));
            }
            else
            {
                if (!context.ModelState.IsValid && !IsAttributeExist(context))
                {
                    ...
                    if (EmptyVal == 0 || valid > 0)
                    {
                        ...
                        context.Result = new BadRequestObjectResult(APIResponseExtensions.GenerateResponse(ApiStatusCode.Status400BadRequest, _message));
                    }
                }
            }
        }
