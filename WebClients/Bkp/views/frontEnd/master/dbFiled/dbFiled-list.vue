<template>
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create DB Filed"  @click="OpenAddNewManage"></Button>
        </template>
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import dbManage from "@/views/frontEnd/master/dbFiled/dbFiled-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(dbManage, "Create Database Field", 0, () => {
            dt.value.reloadGrid();
        });
    };
    const updateIsActive = (data: any) => {
        PatchData(UrlConstants.apiDbFieldMasterUpdateStatus, data, "DB Filed Status Updated successfully.", null, null);
    };

    const gridConfig = ref({
        api: UrlConstants.apiDbFieldMasterList,
        deleteApi: UrlConstants.apiDbFieldMaster,
        filters: [
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
                popupManage.value.OpenModal(dbManage, "Edit Database Field", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("DatabaseFiled_list_"+Date.now())
</script>
