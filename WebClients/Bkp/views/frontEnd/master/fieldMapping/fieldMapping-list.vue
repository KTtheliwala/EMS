<template>
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        <template v-slot:basicfilter>
            <div class="col-12 md:col-3 pb-0">
                <div class="field">
                    ...
                    name="CompanyId" v-model="gridConfig.filters[0].fieldValue"
                    :isWatch="false" 
                    @update:title="gridConfig.filters[0].fieldDisplayValue = $event"
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create Field Mapping"  @click="OpenAddNewManage"></Button>
        </template>
        <template v-slot:columns>
            ...
            <Column field="DbFieldName" header="Database Field" :sortable="true"></Column>     
            <Column field="CompanyFieldName" header="Company Field" :sortable="true"></Column>       
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import fieldMappingManage from "@/views/frontEnd/master/fieldMapping/fieldMapping-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(fieldMappingManage, "Create Field Mapping", 0, () => {
            dt.value.reloadGrid();
        });
    };
    const updateIsActive = (data: any) => {
        PatchData(UrlConstants.apiFieldMappingUpdateStatus, data, "Field Mapping Status Updated successfully.", null, null);
    };

    const gridConfig = ref({
        api: UrlConstants.apiFieldMappingList,
        deleteApi: UrlConstants.apiFieldMapping,
        filters: [
            { searchType: "Filter", fieldName: "CompanyId", fieldValue: null, fieldDisplayName: "Company", opType: SearchOperations.equals },
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
                popupManage.value.OpenModal(fieldMappingManage, "Edit Field Mapping", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("fieldmapping_list_"+Date.now())
</script>
