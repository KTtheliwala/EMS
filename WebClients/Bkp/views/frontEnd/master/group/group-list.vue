<template>
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create Group"  @click="OpenAddNewManage"></Button>
        </template>
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import groupManage from "@/views/frontEnd/master/group/group-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(groupManage, "Create Group", 0, () => {
            dt.value.reloadGrid();
        });
    };
    const updateIsActive = (data: any) => {
        PatchData(UrlConstants.apiGroupUpdateStatus, data, "Group Status Updated successfully.", null, null);
    };

    const gridConfig = ref({
        api: UrlConstants.apiGroupList,
        deleteApi: UrlConstants.apiGroup,
        filters: [
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
                popupManage.value.OpenModal(groupManage, "Edit Group", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("group_list_"+Date.now())
</script>
