<template>
    <common-form ref="frm" :dataId="Id" :actionUrl="UrlConstants.apiGroup" :v$="v$" :data="obj" :successCallBack="successCallback" :failureCallBack="failureCallBack" :successMessage="Msg">
...
<script setup lang="ts">
    ...
    import groupModelDTO, { groupModel } from "@/models/master/groupModel";
    const dialogRef = inject("dialogRef") as any;
    const Id = ref(dialogRef.value.data);
    const obj = ref<groupModel>(new groupModel());
    if (Id.value == 0)
    ...
    onMounted(() => {
        frm.value.SetValidation((data: groupModel) => {
            obj.value = data;
            v$.value = useVuelidate(groupModelDTO.rules, obj.value);
        });
    });
