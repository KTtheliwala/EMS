<template>
    <common-form ref="frm" :dataId="Id" :actionUrl="UrlConstants.apiTransporter" :v$="v$" :data="obj" :successCallBack="successCallback" :failureCallBack="failureCallBack" :successMessage="Msg">
...
<script setup lang="ts">
    ...
    import transpoterModelDTO, { transpoterModel } from "@/models/master/transpoterModel";
    const dialogRef = inject("dialogRef") as any;
    const Id = ref(dialogRef.value.data);
    const obj = ref<transpoterModel>(new transpoterModel());
    if (Id.value == 0)
        obj.value.IsActive = true;
    const v$ = ref();
    const frm = ref();
    const Msg = ref(Id.value > 0 ? "Transporter updated successfully." : "Transporter added successfully.");
    onMounted(() => {
        frm.value.SetValidation((data: transpoterModel) => {
            obj.value = data;
            v$.value = useVuelidate(transpoterModelDTO.rules, obj.value);
        });
    });
