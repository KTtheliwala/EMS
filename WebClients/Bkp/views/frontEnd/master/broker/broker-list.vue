    ...
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        <template v-slot:basicfilter>
            <div class="col-12 md:col-3 pb-0">
                <div class="field">
                    <label class="block w-full">Broker</label>
        ...
        <template v-slot:advancefilter>
            <div class="col-12 md:col-3 pb-0">
                <div class="field">
                    <label class="block w-full">Broker Name</label>
                    <common-input inputclass="form-control" v-model="gridConfig.filters[0].fieldValue"></common-input>
                </div>
            </div>
            <div class="col-12 md:col-3 pb-0">
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create Broker"  @click="OpenAddNewManage"></Button>
        </template>
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import brokerManage from "@/views/frontEnd/master/broker/broker-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(brokerManage, "Create Broker", 0, () => {
            dt.value.reloadGrid();
        });
    };
    const updateIsActive = (data: any) => {
        PatchData(UrlConstants.apiBrokerMasterUpdateStatus, data, "Broker Status Updated successfully.", null, null);
    };

    const gridConfig = ref({
        api: UrlConstants.apiBrokerMasterList,
        deleteApi: UrlConstants.apiBrokerMaster,
        filters: [
            { searchType: "Filter", fieldName: "Name", fieldValue: null, fieldDisplayName: "Broker", opType: SearchOperations.contains },
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
              popupManage.value.OpenModal(brokerManage, "Edit Broker", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("Broker_list_"+Date.now())
</script>
