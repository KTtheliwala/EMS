<template>
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create Company"  @click="OpenAddNewManage"></Button>
        </template>
        <template v-slot:columns>
            ...
            <Column field="MobileNo" header="Mobile" :sortable="true"></Column>
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import companyManage from "@/views/frontEnd/master/company/comany-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(companyManage, "Create Company", 0, () => {
            dt.value.reloadGrid();
        });
    };
    ...
    const gridConfig = ref({
        api: UrlConstants.apiCompanyMasterList,
        deleteApi: UrlConstants.apiCompanyMaster,
        filters: [
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
              popupManage.value.OpenModal(companyManage, "Edit Company", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("company_list_"+Date.now())
</script>
