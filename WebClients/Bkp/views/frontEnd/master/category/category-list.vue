    ...
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        <template v-slot:basicfilter>
            <div class="col-12 md:col-3 pb-0">
                <div class="field">
                    <label class="block w-full">Category Name</label>
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create Category"  @click="OpenAddNewManage"></Button>
        </template>
        <template v-slot:columns>
            ...
            <Column field="ShortDescription" header="ShortDescription" :sortable="true"></Column>            
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import categoryManage from "@/views/frontEnd/master/category/category-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(categoryManage, "Create Category", 0, () => {
            dt.value.reloadGrid();
        });
    };
    const updateIsActive = (data: any) => {
        PatchData(UrlConstants.apiCategoryMastersUpdateStatus, data, "Category Status Updated successfully.", null, null);
    };

    const gridConfig = ref({
        api: UrlConstants.apiCategoryMastersList,
        deleteApi: UrlConstants.apiCategoryMasters,
        filters: [
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
              popupManage.value.OpenModal(categoryManage, "Edit Category", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("category_list_"+Date.now())
</script>