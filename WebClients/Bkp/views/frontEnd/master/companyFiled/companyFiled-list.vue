<template>
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create Company Field"  @click="OpenAddNewManage"></Button>
        </template>
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import companyFiledManage from "@/views/frontEnd/master/companyFiled/companyFiled-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(companyFiledManage, "Create Company Field", 0, () => {
            dt.value.reloadGrid();
        });
    };
    const updateIsActive = (data: any) => {
        PatchData(UrlConstants.apiCompanyFiledUpdateStatus, data, "Company Filed Status Updated successfully.", null, null);
    };

    const gridConfig = ref({
        api: UrlConstants.apiCompanyFiledList,
        deleteApi: UrlConstants.apiCompanyFiled,
        filters: [
            { searchType: "Filter", fieldName: "Name", fieldValue: null, fieldDisplayName: "Name", opType: SearchOperations.contains },
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
                popupManage.value.OpenModal(companyFiledManage, "Edit Company Field", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("companyFiled_list_"+Date.now())
</script>
