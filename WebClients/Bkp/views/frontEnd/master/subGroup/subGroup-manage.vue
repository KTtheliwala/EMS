<template>
    <common-form ref="frm" :dataId="Id" :actionUrl="UrlConstants.apiSubGroup" :v$="v$" :data="obj" :successCallBack="successCallback" :failureCallBack="failureCallBack" :successMessage="Msg">
        <div class="grid guter-change">
            <div class="col-12 md:col-4">
                <div class="field">
                    <label class="block w-full">Group</label>
...
<script setup lang="ts">
    ...
    import subGroupModelDTO, { subGroupModel } from "@/models/master/subGroupModel";
    const dialogRef = inject("dialogRef") as any;
    const Id = ref(dialogRef.value.data);
    const obj = ref<subGroupModel>(new subGroupModel());
    if (Id.value == 0)
        obj.value.IsActive = true;
    const v$ = ref();
    const frm = ref();
    const Msg = ref(Id.value > 0 ? "Group updated successfully." : "Group added successfully.");
    onMounted(() => {
        frm.value.SetValidation((data: subGroupModel) => {
            obj.value = data;
            v$.value = useVuelidate(subGroupModelDTO.rules, obj.value);
        });
    });
