<template>
    <common-grid :config="gridConfig" ref="dt" :filters="filters" :filename="filename" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
        ...
        <template v-slot:right-buttons>
            <Button icon="pi pi-plus-circle" label="Create Sub Group"  @click="OpenAddNewManage"></Button>
        </template>
        <template v-slot:columns>
            ...
            <Column field="Name" header="Sub Group" :sortable="true"></Column>            
...
<script setup lang="ts">
    ...
    import { ref } from "vue";
    import subGroupManage from "@/views/frontEnd/master/subGroup/subGroup-manage.vue";
    ...
    const OpenAddNewManage = () => {
        popupManage.value.OpenModal(subGroupManage, "Create Sub Group", 0, () => {
            dt.value.reloadGrid();
        });
    };
    const updateIsActive = (data: any) => {
        PatchData(UrlConstants.apiGroupUpdateStatus, data, "Sub Group Status Updated successfully.", null, null);
    };

    const gridConfig = ref({
        api: UrlConstants.apiSubGroupList,
        deleteApi: UrlConstants.apiSubGroup,
        filters: [
    ...
    const gridMenuItems = ref([
        {
            ...
            label: "Edit",
            callback: (data: any) => {
                popupManage.value.OpenModal(subGroupManage, "Edit Sub Group", data.Id, dt.value.reloadGrid);
            },
            visible:true, 
        },
        {
    ...
    const filename = ref("subGroup_list_"+Date.now())
</script>
