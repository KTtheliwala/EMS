    ...
    <div class="content-wrap">
        <common-grid :config="gridConfig" ref="dt" :filters="filters" :hideExportBtn="false" :AdvSearchBtnClass="'advance-search-filter-only-for-mobile'">
            ...
            <template v-slot:columns>
                <Column header="Action" class="action-dot-button" frozen alignFrozen="left" v-if="(Page?.IsEdit ?? false )">
                    <template #body="columns">
                        <div>
                            <Button class="p-button-aux smallIconBtn" v-if="(Page?.IsEdit ?? false )"  label="Edit" @click="OpenAddNewManage(columns.data)" />
                        </div>
                    </template>
                </Column>
...
<script setup lang="ts">
    ...
    const Page = AuthService.GetPagePermission('AdminTools','SystemSetting')
