<template>
    <common-form ref="frm" :dataId="Id" :actionUrl="UrlConstants.apirole" :v$="v$" :data="obj" :successCallBack="successCallback" :failureCallBack="failureCallBack" :successMessage="Msg">
...
<script setup lang="ts">
    ...
    import roleModelDTO, { roleModel } from "@/models/admintools/roleModel";
    const dialogRef = inject("dialogRef") as any;
    const Id = ref(dialogRef.value.data);
    const obj = ref<roleModel>(new roleModel());
    if (Id.value == 0)
    ...
    onMounted(() => {
        frm.value.SetValidation((data: roleModel) => {
            obj.value = data;
            v$.value = useVuelidate(roleModelDTO.rules, obj.value);
        });
    });
